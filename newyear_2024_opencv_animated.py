...
"""
Happy New Year 2024 with Gifts, Animated Squares and Balls in the Tree!
Turkish and English Texts Together
...
vout = cv2.VideoWriter('newyear_2024_video.mp4', fourcc, 8, (795,795))
...
while True:
    ...
    cv2.rectangle(image, (90,150), (710,70), (0,0,0), -1)
    if txt % 2 == 1:
        cv2.putText(image, "MUTLU YILLAR!", (110,137), FONT, 2.3, (0,brt,brt), 4, AA)
    elif txt % 2 == 0:
        cv2.putText(image, "HAPPY NEW YEAR!", (95,135), FONT, 2, (0,brt,brt), 4, AA)
    cv2.putText(image, "2", (90,305), FONT, 4.2, (0,brt,brt), 6, AA)
    cv2.putText(image, "0", (90,410), FONT, 4.2, (0,brt,brt), 6, AA)
    cv2.putText(image, "2", (90,515), FONT, 4.2, (0,brt,brt), 6, AA)
    cv2.putText(image, "4", (90,620), FONT, 4.2, (0,brt,brt), 6, AA)
    
    cv2.putText(image, "2", (620,305), FONT, 4.2, (0,brt,brt), 6, AA)
    cv2.putText(image, "0", (620,410), FONT, 4.2, (0,brt,brt), 6, AA)
    cv2.putText(image, "2", (620,515), FONT, 4.2, (0,brt,brt), 6, AA)
    cv2.putText(image, "4", (620,620), FONT, 4.2, (0,brt,brt), 6, AA)
    ...
    cv2.circle(image, (360,295), 12, COLORS_DARK[(s//2+c+7)%8], -1, AA)
    cv2.circle(image, (360,295), 9, COLORS[(s//2+c+7)%8], -1, AA)
    cv2.circle(image, (440,295), 12, COLORS_DARK[(s//2+c+4)%8], -1, AA)
    cv2.circle(image, (440,295), 9, COLORS[(s//2+c+4)%8], -1, AA)
    cv2.circle(image, (400,260), 12, COLORS_DARK[(s//2+c+0)%8], -1, AA)
    cv2.circle(image, (400,260), 9, COLORS[(s//2+c+0)%8], -1, AA)
    ...
    cv2.imshow("MUTLU YILLAR / HAPPY NEW YEAR 2024", image)
    vout.write(image)
    s += 1
    brt += ch
    if brt == 75:
